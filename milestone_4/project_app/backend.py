...
def display_corpus(keyword, entity):
    if entity == "judge":
        ...
        return HTMLResponse(put_in_table(find_matching_documents(keyword, entity, entity_name)))
    if entity == "appellant":
...
def put_in_table(doc_dict):
    return '<table class="table table-hover table-dark"><thead><tr><th scope="col">#</th><th scope="col">Matched Document</th><th scope="col">Link to the Document</th></tr></thead><tbody>' + create_row(doc_dict) + '</tbody></table>'
